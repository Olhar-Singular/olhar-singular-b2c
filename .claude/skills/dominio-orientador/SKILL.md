...
- **Estorno em autoatendimento (2026-09-18).** `refund-last-charge` (JWT do próprio usuário, sem
  `userId` de admin) devolve a última cobrança aprovada ao cartão: `refund_last_charge` (RPC) acha
  a invoice elegível (assinatura viva, ou a mais recente `cancelled` com `cancelled_at` há menos de
  **30 dias**: `REFUND_WINDOW_DAYS` em `lib/domain/subscriptionUi.ts`, espelhado no `canRefundLastCharge`
  que decide o botão) e trava a linha; a **ordem importa**: MP primeiro
  (`POST /v1/payments/{id}/refunds`, `X-Idempotency-Key = "refund:" + invoice.id`: não-2xx devolve
  502 **sem escrever nada**), depois `PUT /preapproval/{id} { status:'cancelled' }` (best effort,
  loga e segue), só então `confirm_refund` (RPC) grava `refunded_at`/`mp_refund_id`, zera o balde do
  plano com uma linha `refund_clawback` no ledger (extras ficam) e cancela a assinatura localmente.
  `confirm_refund` é idempotente por `refunded_at` (replay depois de um confirm que falhou repete o
  POST com a mesma chave e não zera o balde de novo). Cliente: `useLastCharge` (lê
  `subscription_invoices` direto) + `useRefundLastCharge` (invoca `refund-last-charge`), botão e
  diálogo no `SubscriptionCard`, rótulo `refund_clawback` no extrato (`CreditsPage`). No Admin,
  **"Teste (cartão)" (`trial_card`) não pode "Estender"**: `admin_extend_trial` devolve
