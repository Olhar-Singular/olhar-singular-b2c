...
Plataforma educacional B2C. **Educadores adaptam atividades pedagógicas (provas, exercícios) para alunos com barreiras de aprendizagem (ex.: TEA), usando IA.** Monetização por **créditos em dois baldes** (plano mensal, que zera na renovação, e extras avulsos, que nunca expiram; Pix e cartão via Mercado Pago, os dois inline). **Não há cadastro público nem adaptação grátis**: contas nascem pelo checkout pago, pelo **teste grátis com cartão** (7 dias/50 créditos, `/assinar?trial=1`, cobra o plano mais barato no 8º dia) ou por convite do admin (trial sem cartão ou cortesia). Redesign em curso (assinatura): `docs/superpowers/specs/2026-09-11-pagamentos-assinatura-mp-design.md`.
...
- **Assinatura (Fase 3), regras que mordem.** (1) **Ativação otimista**: `subscribe` chama `activate_subscription` assim que o `POST /preapproval` volta `authorized`, gravando uma fatura sintética `activation:<id>` e carregando o balde do plano; a **1ª cobrança** chega depois pelo tópico `subscription_authorized_payment` e `renew_subscription` só a **confirma** (`first_payment_confirmed`), sem zerar; se ela vier `rejected`, é `clawback` (balde zerado, `subscriptions.status = 'rejected'`). (2) **Só `payment.status === 'approved'` é dinheiro**: `status: 'processed'` no `authorized_payment` significa apenas "o MP terminou de tentar"; `recycling` é retry em curso (`interpretAuthorizedPayment` em `_shared/mpPreapproval.ts`). Fatura com `status = 'scheduled'` ou `payment_status` em `pending`/`in_process`/`authorized` só é espelhada e devolve `pending` (nunca `clawback`/`past_due`: migration `20260918000001`); uma fatura `approved` com `amount_brl = 0` (validação de cartão do MP) devolve `ignored` sem converter nem renovar nada; e `payment_status` em `refunded`/`charged_back` (estorno externo pelo painel do MP ou chargeback do adquirente) tem as mesmas consequências do `confirm_refund` em vez de `past_due`/`clawback` (migration `20260920000002`, ver a entrada de estorno em autoatendimento abaixo). (3) **Renovação** zera e recarrega quando `debit_date >= period_end - 1 dia`; a transição da fatura para `granted_at` é a única concessão (idempotente por `subscription_invoices.id`). (4) **Uma assinatura viva por usuário** (índice parcial em `authorized|past_due|paused`); `pending` com mais de 15 min é abandonada pelo `subscribe` seguinte. (5) **O POST /preapproval nunca é repetido às cegas**: em falha de rede, `GET /preapproval/search?external_reference=<subscriptions.id>` decide. (6) **Cancelar fala com o MP primeiro** (`PUT /preapproval/{id} {status:'cancelled'}`) e só então `cancel_subscription_local`; os créditos do período pago ficam até `plan_period_end` (expiração preguiçosa). (7) Webhook: `subscription_preapproval` → `GET /preapproval/{id}` → `sync_subscription_status` (espelho de `authorized|paused|cancelled`); a linha é achada pelo `external_reference` (uuid) ou pelo `mp_preapproval_id`. (8) **Dinheiro em linha fechada** (migration `20260916000002`): cobrança aprovada para uma linha `rejected`/`cancelled` **reativa com a cota** se o usuário não tem outra viva (`renew_subscription` → `reactivated`); se tem, devolve `paid_while_closed` e o webhook cancela o preapproval órfão no MP (`closeOrphan` em `_shared/subscriptionActions.ts`); `sync_subscription_status` propaga `duplicate_live_subscription` (mesmo tratamento). O `subscribe` cancela no MP as pendentes abandonadas (>15 min) antes de fechá-las. (9) Toda chamada ao MP usa `_shared/mpHttp.ts` (timeout 15s): cartão que estoura o timeout fica `pending` e o webhook decide. Não há `statement_descriptor` no `/preapproval`: o nome na fatura é configuração da conta MP (runbook da spec, seção 6). Cobertura: `subscriptions_schema.test.sql`, `subscription_rpcs.test.sql`, `_shared/subscribeFlow.test.ts`, `_shared/subscriptionActions.test.ts`.
- **Funil pagar-primeiro (Fase 4).** `subscribe` é público: JWT válido = fluxo logado; senão exige `account { fullName, email, termsVersion }`, cria a conta com senha aleatória e `email_confirm: true` (`_shared/accountProvision.ts` → `runAnonymousCheckout`), e **nunca devolve sessão** (decidido em 2026-09-13): com a conta nova, o cliente chama `signInWithOtp({ email, options: { emailRedirectTo: '/definir-senha', shouldCreateUser: false } })` e mostra "confira seu e-mail", seja o cartão aceito, pendente ou recusado (decisão 14: a conta fica). O link do e-mail abre a sessão e cai em `/definir-senha` (`set-initial-password`: senha, `must_set_password = false`, `auth.admin.signOut(jwt, 'others')`). Rate limit em `checkout_attempts` via `record_checkout_attempt` (HMAC de e-mail normalizado e IP com `CHECKOUT_HASH_SECRET`; 5/e-mail e 10/IP por hora, disjuntor com mais de 20 recusas em 10 min → 503). Uma tentativa `pending` com menos de 15 min bloqueia outra (`attempt_in_progress`); se a ativação for recusada (`duplicate_live_subscription`), o preapproval órfão é cancelado no MP. CPF só se válido e só se `profiles.cpf IS NULL`; `terms_version`/`terms_accepted_at` só após cartão aceito. Convite do admin cai em `/redefinir-senha?convite=1` ("Crie sua senha", mantém a sessão; template `supabase/templates/invite.html`). Guarda de copy: `src/test/copyGuard.test.ts` (gratuit*, nunca expiram, Stripe; `grátis` só numa linha com teste/testar).
- **Teste grátis com cartão (2026-09-15).** `subscribe` com `trial: true` (só anônimo; logado ⇒ 400 `trial_requires_new_account`): plano = mais barato ativo não `admin_only` (`loadCheapestPublicPlan`, nunca o `planSlug` do cliente), **um trial por CPF** via RPC `trial_used_by_cpf` checada **antes** do `createUser` (409 `trial_used`; CPF inválido ⇒ 400 `cpf_required`), e a linha de `subscriptions` nasce com **`trial_ends_at`** = `now + 7d` (o mesmo `auto_recurring.start_date` enviado ao MP; `reason` cortado em **60 chars**, limite do MP). **A linha carrega a intenção**: `activate_subscription` (mesma assinatura) ramifica pela coluna: 50 créditos no balde do plano até `trial_ends_at`, `access_kind = 'trial'`, sem cota, sem invoice `activation:`; o 1º `subscription_authorized_payment` aprovado vira `renew_subscription → trial_converted` (cota via `apply_plan_quota`, `subscriber`, período de 1 mês a partir do débito; o `mp-webhook` conta como `subscription_renewed`); recusado ⇒ `clawback`. Entre a criação do preapproval e o débito do 8º dia, uma fatura ainda não liquidada (`status = 'scheduled'` ou `payment_status` em `pending`/`in_process`/`authorized`) só é espelhada (`pending`, nunca `clawback`) e uma aprovada com `amount_brl = 0` (validação de cartão) é `ignored`, sem converter o trial (migration `20260918000001`). Cartão recusado no teste (síncrono, `runAnonymousCheckout`): a conta recém-criada é **apagada** (`auth.admin.deleteUser`) pra o mesmo e-mail poder tentar outro cartão sem cair em `email_exists`; a decisão 14 ("a conta fica") vale só pro funil pago. `cancel_subscription_local` dentro do trial (`trial_ends_at` não nulo e `first_payment_confirmed = false`) **zera os créditos na hora** (`plan_reset`, `trial_closed: true`); fora dele, expiração preguiçosa como sempre. No cliente: `isCardTrial(sub)` (`lib/domain/subscriptionUi.ts`) muda `SubscriptionCard`, `AccessBanner` e o toast do `useCancelSubscription({ trial })`; `trial_used` volta com `code` (`parseInvokeFailure`) e a `SubscribePage` oferece o plano pago com o mesmo cartão (o token não foi consumido: o servidor recusou antes de falar com o MP). O pagamento `card_validation` (R$ 0,00 no sandbox) não tem `external_reference` de compra e é ignorado pelo webhook. Sandbox: Visa `4235 6477 2802 5682` (a Master `5031 4332…` é recusada para recorrência) e comprador `BUYER_TEST_EMAIL_MP`. Cobertura: pgTAP `trial_with_card.test.sql`, `renew_unsettled_guard.test.sql`; `subscribeFlow.test.ts`, `accountProvision.test.ts`.
- **Estorno em autoatendimento (2026-09-18, guards reforçados 2026-09-18).** `refund-last-charge`
  (JWT do próprio usuário, sem `userId` de admin) devolve a última cobrança aprovada ao cartão:
  `refund_last_charge` (RPC) acha a invoice elegível (assinatura viva, ou a mais recente
  `cancelled` com `cancelled_at` há menos de **30 dias**: `REFUND_WINDOW_DAYS` em
  `lib/domain/subscriptionUi.ts`, espelhado no `canRefundLastCharge` que decide o botão),
  `amount_brl > 0` (a fatura de validação de cartão do MP, espelhada com `amount_brl = 0`, nunca
  é oferecida nem aparece como "última cobrança") e nunca uma cobrança mais antiga que uma já
  estornada (não é mais uma trava vitalícia: um `PUT /preapproval` que falhou no MP não pode
  travar o usuário pra sempre) e trava a linha; a **ordem importa**: MP primeiro
  (`POST /v1/payments/{id}/refunds`, `X-Idempotency-Key = "refund:" + invoice.id`); num não-2xx,
  antes de desistir, confere o estado do estorno no MP (`GET /v1/payments/{id}`: um timeout não
  prova que nada aconteceu do outro lado); só então `PUT /preapproval/{id} { status:'cancelled' }`
  (best effort, **uma retentativa** antes de logar `ALERT preapproval not cancelled at MP`,
  convenção de `closeOrphan`), e `confirm_refund` (RPC) grava `refunded_at`/`mp_refund_id`, zera
  o balde do plano com uma linha `refund_clawback` no ledger (extras ficam) e cancela a assinatura
  localmente. Uma recusa 4xx do MP (`permanent: true`) vira **422** no `index.ts` ("Fale com o
  suporte"); uma falha transitória continua 502 (o usuário tenta de novo). `confirm_refund` é
  idempotente por `refunded_at` (replay depois de um confirm que falhou repete o POST com a
  mesma chave e não zera o balde de novo). Cliente: `useLastCharge` (lê `subscription_invoices`
  direto, `.gt("amount_brl", 0)`) + `useRefundLastCharge` (invoca `refund-last-charge`), botão e
  diálogo no `SubscriptionCard`, rótulo `refund_clawback` no extrato (`CreditsPage`). **A linha
  fica visível depois do estorno**: `useLastCharge` e o `admin-dashboard` filtram
  `.or("payment_status.eq.approved,refunded_at.not.is.null")` em vez de só `eq("payment_status",
  "approved")`, porque um `authorized_payment` que chega depois pode sobrescrever
  `payment_status` com `'refunded'` e sumir com o "Estorno solicitado". No Admin, **"Teste
  (cartão)" (`trial_card`) não pode "Estender"**: `admin_extend_trial` devolve
  `{ success:false, error:'card_trial' }` porque o MP **ignora** mudanças em `auto_recurring.start_date`
  (e em `free_trial`) num `PUT /preapproval/{id}` já autorizado: o débito do dia 8 acontece de
  qualquer jeito, então estender só adiaria o paywall sem adiar a cobrança. **Estorno externo**
  (rodada 2b, migration `20260920000002`): um estorno feito no painel do MP ou um chargeback do
  adquirente (`payment_status` `refunded`/`charged_back` no `authorized_payment`) faz o
  `renew_subscription` chamar `confirm_refund` por baixo (`refunded_externally`) e o
  `mp-webhook` cancela a preapproval no MP (best effort, `cancelAfterExternalRefund` em
  `_shared/subscriptionActions.ts`); um timeout do MP em `postRefund`/`getPaymentRefundState`
  (`_shared/refundDeps.ts`) vira `status: 0`/`null`, nunca `permanent: true`, então o
  `refund-last-charge` responde **502** (retentável) em vez de 422 ("fale com o suporte"). O
  `admin-dashboard` soma `refund_count` (faturas com `refunded_at`) por usuário, mostrado como
  "estornos: N" na `UsersTable`.
- **Analytics (Fase 6).** `src/lib/analytics/`: Consent Mode v2 com tudo `denied` até o `ConsentBanner` (montado no `AppRoot`) decidir; `bootAnalytics` roda a cada navegação (defaults uma vez, GTM via `loadGtm` só com `VITE_GTM_ID` e **nunca** em `/assinar`/`/definir-senha`, atribuição em `sessionStorage`: `utm_*`/`gclid`/`fbclid`/`referrer` sempre, `_ga`/`_fbp`/`_fbc` só com consentimento). Eventos tipados em `events.ts` (`view_item_list`, `select_item`, `begin_checkout`, `add_payment_info`, `subscription_started`, `purchase` só no cartão síncrono, `paywall_shown`, `consent_updated`); `event_id` = id da compra/assinatura. `SubscribePage` envia a `attribution` no `subscribe` (gravada em `subscriptions.attribution`). Servidor: `_shared/analyticsEvents.ts` (GA4 MP + Meta CAPI, e-mail SHA-256 só com `ad_user_data`, sem `client_id` sem `analytics_storage`, no-op sem os 4 secrets, timeout 3s, nunca lança) chamado por `subscribe`, `create-card-payment`, `mp-webhook` (purchase, renewed, payment_failed) e `cancel-subscription`. Nada de e-mail/CPF no `dataLayer`.
