...
Plataforma educacional B2C. **Educadores adaptam atividades pedagógicas (provas, exercícios) para alunos com barreiras de aprendizagem (ex.: TEA), usando IA.** Monetização por **créditos** (cartão e Pix, ambos via Stripe). 1ª adaptação grátis; demais debitam crédito.
...
| **Créditos** | `CreditsPage`, `CreditsSuccessPage` | `useCredits` | `create-stripe-checkout`, `stripe-webhook` | Compra (cartão e Pix). RPCs `deduct_credits`/`grant_credits`. |
...
- Pagamentos: **Stripe é o único provedor** — cartão e Pix saem do mesmo `create-stripe-checkout`, chaveados pelo campo `method` do body (`_shared/stripeCheckoutParams.ts`). `credit_purchases.payment_method` (`'card'|'pix'`) grava qual foi; `provider` é sempre `'stripe'` (`'mercadopago'` só sobrevive em linhas históricas).
- **Pix é assíncrono e essa é a pegadinha**: `checkout.session.completed` chega com `payment_status: "unpaid"` (é só o QR aparecendo na tela); o dinheiro só é confirmado em `checkout.session.async_payment_succeeded`. `extractCheckoutGrant` aceita os dois eventos mas **sempre** exige `payment_status === "paid"` — afrouxar esse gate distribui crédito de graça. `async_payment_failed`/`expired` → `extractCheckoutFailure` marca a compra `rejected` (nunca rebaixa uma já `approved`). QR expira em 1h (`PIX_EXPIRES_AFTER_SECONDS`). Esses 3 eventos precisam estar assinados no dashboard da Stripe, senão Pix nunca credita.
