/**
 * AppearancePopover — controle de Aparência do documento (plano §6.5, D7/D12).
 *
 * Atua sobre os PAGE TOKENS do documento inteiro (fonte, tamanho, espaçamento
 * entre blocos), não sobre estilo por bloco — assim a paridade com o PDF é
 * automática (o PDF lê os mesmos tokens). Emite `Partial<PageStyle>`; o
 * `StepReview` mescla no `pageStyle` e persiste.
 *
 * Unidades: o usuário pensa em px (tamanho 11–28, espaçamento 8–40); o
 * `pageStyle.fontSize` é armazenado em pt (paridade direta com o PDF). A
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button size="sm" variant="ghost" className="shrink-0 text-surface-ink-soft hover:text-surface-ink">
          <Type className="mr-1 h-4 w-4" /> Aparência
        </Button>
      </PopoverTrigger>
