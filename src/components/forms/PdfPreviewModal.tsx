  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-4xl max-h-[95vh]">
        <DialogHeader>
          <DialogTitle className="truncate">{file?.name || "Prévia do PDF"}</DialogTitle>
        </DialogHeader>
                ...
                {cropping && cropRect && (
                  ...
                  >
                    {[{ top: -4, left: -4 }, { top: -4, right: -4 }, { bottom: -4, left: -4 }, { bottom: -4, right: -4 }].map((pos, i) => (
                      <div key={i} className="absolute w-2 h-2 bg-primary rounded-full" style={pos as React.CSSProperties} />
                    ))}
                  </div>
                )}
              </div>
            ...
            {cropping && (
              <>
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => { setCropping(false); setCropStart(null); setCropEnd(null); }}
                >
                  Cancelar
                </Button>
