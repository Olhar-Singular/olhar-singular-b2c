...
type Stage =
  ...
  | { kind: "approved"; purchaseId: string }
...
export default function CardPaymentDialog({ pkg, onOpenChange }: Props) {
  ...
  // Credit lands on the server; the client only has to catch up once per purchase.
  const refreshedFor = useRef<string | null>(null);
  function settleApproved(purchaseId: string) {
    if (refreshedFor.current === purchaseId) return;
  ...
  useEffect(() => {
    ...
    if (polled.status === "approved") {
      setStage({ kind: "approved", purchaseId: stage.purchaseId });
      settleApproved(stage.purchaseId);
    } else if (polled.status === "rejected" || polled.status === "cancelled") {
      setStage({ kind: "rejected", message: "O pagamento não foi aprovado. Tente outro cartão ou pague com Pix." });
    }
    // settleApproved reads refs/props that are stable for this purchase.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [polled, stage]);

  async function handleSubmit(card: CardFormDataView) {
    if (!pkg) return;
    const result = await payment.mutateAsync({ packageId: pkg.id, card });
    if (result.status === "approved") {
      setStage({ kind: "approved", purchaseId: result.purchaseId });
      settleApproved(result.purchaseId);
    } else if (result.status === "rejected") {
      setStage({
        kind: "rejected",
        message: result.message ?? "O pagamento não foi aprovado. Tente outro cartão ou pague com Pix.",
      });
    } else {
