...
> ✅ Provider confirmado (2026-06-03): a Lovable **já foi removida**; `_shared/aiConfig.ts` usa `AI_API_KEY` (Google AI Studio) direto em `https://generativelanguage.googleapis.com/v1beta/openai`. Esse endpoint OpenAI-compat do Google **suporta structured outputs** (`response_format: json_schema` / nativo `responseSchema`) — o pilar de saída estruturada é viável já. (Q24 resolvido.) Resíduos: um teste protetor que trava o retorno da Lovable (manter) e o spec histórico `2026-06-01-error-handling-lovable-cleanup-design.md`.
